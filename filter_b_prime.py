"""
B_ 이미지와 A2 이미지 유사도 측정 후
너무 비슷한 B_ 이미지 필터링 (삭제 또는 이동)

실행:
python filter_b_prime.py --data_dir ../mfvs_dataset --threshold 0.7 --dry_run
...
def parse_args():
    ...
    parser.add_argument("--threshold", type=float, default=0.7,
                        help="B'와 A2 유사도가 이 값 이상이면 필터링 (기본 0.7)")
...
def main():
    ...
    for identity_dir in tqdm(identity_dirs, desc="Filtering"):
        ...
        # B_ 서브디렉토리 순회 (A2 이미지명별 디렉토리)
        for b_subdir in b_dir.iterdir():
            if not b_subdir.is_dir():
                continue
    ...
    print(f"\n완료: 전체 {total_checked}장 중 {total_filtered}장 필터링")
